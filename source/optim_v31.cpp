...
void print(uint8_t *buffer, size_t dim0, size_t dim1)
{
...
public:
    FigureProcessor(size_t size, size_t seed = 0) : size(size), buffer_size(size * size)
    {
        ...
        figure = static_cast<uint8_t *>(aligned_alloc(32, sizeof(uint8_t) * (buffer_size + 4 * (size + 1))));    // 增加 padding
        result = static_cast<uint8_t *>(aligned_alloc(32, sizeof(uint8_t) * (buffer_size)));
...
#undef SET_NEXTROW
    ...
    inline __v16hi load_epi8_cvtepi16(const uint8_t *src)
    {
        // _mm_load_epi64(reinterpret_cast<const long long *>(src));  // AVX512 needed
        __m128i load_buffer = _mm_loadu_si128(reinterpret_cast<const __m128i *>(src));   // SSE2
        return _mm256_cvtepu8_epi16(load_buffer);
    }

    inline void store_epi8_cvtepi16(uint8_t *dst, __v16hu &v)
    {
        // __m128i store_buffer = _mm256_cvtepi16_epi8(v);  // AVX512 needed
        static const __m256i zero = _mm256_setzero_si256();
        __v32qu buffer256 = _mm256_packus_epi16(v, zero);
    ...
    inline void gaussianFilterBlock(uint8_t *src, uint8_t *dst)
    {
        ...
        result = load_epi8_cvtepi16(src);
        b1 = load_epi8_cvtepi16(src + 1);
        c1 = load_epi8_cvtepi16(src + 2);
        a2 = load_epi8_cvtepi16(src + size + 2);
        b2 = load_epi8_cvtepi16(src + size + 3);
        c2 = load_epi8_cvtepi16(src + size + 4);
        a3 = load_epi8_cvtepi16(src + 2 * (size + 2));
        b3 = load_epi8_cvtepi16(src + 2 * (size + 2) + 1);
        c3 = load_epi8_cvtepi16(src + 2 * (size + 2) + 2);
        ...
        store_epi8_cvtepi16(dst, result);
    }
    ...
    void powerLawTransformation()
    {
        ...
        for (size_t i = 0; i < size; ++i)
        {
            uint8_t *result_ptr = result + i * size;
            uint8_t *figure_ptr = &ELEMENT_FIGURE(i, 0);
            ...
            while(result_ptr < end)
            {
                ...
                load_reg = _mm_loadu_si128(reinterpret_cast<const __m128i *>(figure_ptr));   // SSE2
                idx_lo = _mm256_cvtepu8_epi32(load_reg);
                load_reg = _mm_srli_si128(load_reg, 8);
                ...
                store_epi8_cvtepi16(result_ptr, val);
                result_ptr += 16;
                figure_ptr += 16;
            }
        }
