...
public:
    FigureProcessor(size_t size, size_t seed = 0) : size(size), buffer_size(size * size)
    {
        ...
        figure = static_cast<uint_fast8_t *>(malloc(buffer_size));
        result = static_cast<uint_fast8_t *>(malloc(buffer_size));
        for (size_t i = 0; i < buffer_size; ++i)
        {
            figure[i] = static_cast<uint_fast8_t>(distribution(gen));
            result[i] = 0;
        }
    }
    ...
    void powerLawTransformation()
    {
        ...
        for (size_t i = 0; i < buffer_size; ++i)
        {
            ...
            result[i] = static_cast<uint_fast8_t>(
